...
todosDB = [{"id": 1, "task": "Do something"}, {"id": 2, "task": "Do something else"}]
...
createTodoCommand = todosCtrlr.model(
    "Create Todo command",
    {"task": fields.String(required=True, description="Todo details")},
)

updateTodoCommand = todosCtrlr.model(
    "Update Todo command",
    {"task": fields.String(required=True, description="Todo details")},
)

todoDto = todosCtrlr.model(
    "Todo DTO",
    {
...
api.add_namespace(todosCtrlr, path="/todos")
